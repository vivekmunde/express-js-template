...
import { getSystemUserAccessibleRoles } from '@/services/rbac/system/get-user-accessible-roles';
import { TProtectedRequestContext } from '@/types/request';
import { TResponseBody } from '@/types/response-body';
import { Role } from '@prisma/client';
import { Request, Response } from 'express';
import { TGetRole, TGetRolesResponseData, TGetRolesResponseErrorCode } from './interfaces';
...
const GET = async (req: Request, res: Response, context: TProtectedRequestContext) => {
  return ifValidListSearchParams<'name'>(req, res, ['name'], async (args) => {
    const accessibleRoles = await getSystemUserAccessibleRoles(context.sessionUser.id);
    ...
    const responseBody: TResponseBody<TGetRolesResponseData, TGetRolesResponseErrorCode> = {
      data: {
