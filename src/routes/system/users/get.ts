...
const GET = async (req: Request, res: Response) => {
  return ifValidListSearchParams<'name' | 'email'>(req, res, ['name', 'email'], async (args) => {
    ...
    const roleIds = [...new Set(usersData.flatMap((user) => user.rbacGlobalRoleIds ?? []))];
    ...
    const usersWithRoles = usersData.map((user) => ({
      ...
      roles: user.rbacGlobalRoleIds.reduce<{ id: string; name: string }[]>((acc, roleId) => {
