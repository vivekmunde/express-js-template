...
import { TPutDocumentStatusResponseData } from './interfaces';
import { getUpdateDocumentStatusValidationSchema } from './validation-schema';

const PUT = async (req: Request, res: Response, context: TProtectedRequestContext) => {
  return ifValidRequestValues(
    req,
    res,
    getUpdateDocumentStatusValidationSchema(req),
    async (data) => {
      ...
      return ifDocumentStatusExists(
        ...
        async () => {
          ...
          const responseBody: TResponseBody<TPutDocumentStatusResponseData> = {
            data: {
              id: updatedDocumentStatusData.id,
              name: updatedDocumentStatusData.name,
              description: updatedDocumentStatusData.description ?? undefined,
              color: updatedDocumentStatusData.color ?? undefined,
            },
          };
